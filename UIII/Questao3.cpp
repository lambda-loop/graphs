 ...
 * Utilizada aqui para inserir um indivíduo "bom" na população inicial (elitismo inicial).
 */
 ...
 * Com uma probabilidade mutationRate, escolhe dois genes aleatórios e os troca de lugar.
 * @param tour Cromossomo a sofrer mutação (passado por referência para alteração direta).
 ...
 * Executa todo o processo evolutivo (População Inicial -> Avaliação -> Seleção -> Crossover -> Mutação).
 ...
 */
std::pair<double, std::vector<int>> runGeneticAlgorithm(const std::vector<std::vector<double>>& matrix, 
...
int main() {
    ...
    for (const auto& entry : fs::directory_iterator(folderPath)) {
        if (entry.is_regular_file() && entry.path().extension() == ".csv") {
            ...
            if (inst.n >= 6) { 
                ...
                std::cerr << "[Aviso] Arquivo " << entry.path().filename().string() << " ignorado." << std::endl;
            }
        }
    }
    ...
    std::ofstream txtOut("resultados_questao3.txt");
    ...
    for (const auto& inst : instances) {
        ...
        for (int run = 0; run < ITERATIONS; ++run) {
            ...
            auto [runBestCost, runBestTour] = runGeneticAlgorithm(inst.matrix, POP_SIZE, GENERATIONS, CROSSOVER_RATE, MUTATION_RATE);
    ...
    if (txtOut.is_open()) {
        txtOut.close();
        std::cout << "Resumo gerado no arquivo 'resultados_questao3.txt'." << std::endl;
    }
