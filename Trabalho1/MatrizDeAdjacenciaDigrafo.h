...
template <typename T> struct MatrizDeAdjacenciaDigrafo {
  ...
  MatrizDeAdjacenciaDigrafo() : numVertices(0) {}

  MatrizDeAdjacenciaDigrafo(vector<vector<int>> m, vector<T> nomes)
  ...
  // Construtor OBRIGATORIAMENTE para Digrafo
  MatrizDeAdjacenciaDigrafo(string caminhoDoGrafo) {
  ...
  void adicionarVertice(T novoVertice, vector<T> vizinhos) {
    ...
    for (const auto &vizinho : vizinhos) {
      if (nomeParaIndice.count(vizinho)) {
        ...
        // Arco saindo do novo vértice para o vizinho
