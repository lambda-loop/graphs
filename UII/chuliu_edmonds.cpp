...
vector<int> chuLiuEdmondsRecursivo(GrafoMatrizInc<int> g, int raiz, vector<int> ids_originais) {
    ...
    for (int i = 0; i < g.vertices; i++) map_node[i] = i;
    for (int u : ciclo) map_node[u] = pseudo_no_k;

    // Encontra a aresta de menor peso
    double min_custo_ciclo = 1e9;
    for (int r : ciclo) {
        if (peso_pai[r] < min_custo_ciclo) min_custo_ciclo = peso_pai[r];
    }
    ...
    for (size_t e = 0; e < g.matriz.size(); e++) {
        ...
        for (int i = 0; i < g.vertices; i++) {
            if (g.matriz[e][i] < 0) u = i;
            if (g.matriz[e][i] > 0) { v = i; peso = g.matriz[e][i]; }
        }
        if (u == -1 || v == -1) continue;
        ...
        nova_linha[map_u] = -1;
        nova_linha[map_v] = static_cast<int>(novo_peso);
    ...
    vector<int> mst_contraida = chuLiuEdmondsRecursivo(g_contraido, raiz, novos_ids);

    // Reconstrução do ciclo original removendo a aresta redundante
    vector<int> resultado_final = mst_contraida;
    
    // Identifica qual aresta que entra no ciclo (pseudo_no_k) foi selecionada na MST contraída.
    // O nó 'v' que recebe essa aresta é o ponto onde o ciclo original será "quebrado".
    int no_quebrado_j = -1; 
    for (int id_sel : mst_contraida) {
        int idx_local = -1;
        // Dado um id, Compara grafo(id) com grafo contraido (id)
        for (size_t e = 0; e < ids_originais.size(); e++) {
            if (ids_originais[e] == id_sel) { idx_local = e; break; }
        }
        if (idx_local != -1) {
            int u = -1, v = -1;
            // Identifica origem e destino da aresta original
            for (int i = 0; i < g.vertices; i++) {
                if (g.matriz[idx_local][i] < 0) u = i;
                if (g.matriz[idx_local][i] > 0) v = i;
            }
...
GrafoMatrizInc<int> generateMinimumSpanningTree(const string& arquivo, int raiz = 0) {
    ...
    vector<int> arestas_finais_ids = chuLiuEdmondsRecursivo(grafo_original, raiz, ids_iniciais);

    GrafoMatrizInc<int> mst = grafo_original;
    mst.matriz.clear(); 

    for (size_t e = 0; e < grafo_original.matriz.size(); ++e) {
        if (find(arestas_finais_ids.begin(), arestas_finais_ids.end(), static_cast<int>(e)) != arestas_finais_ids.end()) {
